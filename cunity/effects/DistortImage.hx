...
* adapted to general Tesselation/Distortion Engine 
...
class DistortImage {
	...
	*/
	public function new (	target_container:DisplayObject, source:BitmapData,
							?vseg:Int=3, ?hseg:Int=3, ?offsetRect:Rectangle) {
		
		if (Std.is (target_container, Shape) || Std.is (target_container, Sprite)) {
			_container = cast target_container;
			_container.addEventListener(MouseEvent.MOUSE_MOVE, showId,false,0,true);
		}
		else {
		...
		/*else if (Std.is (source, DisplayObject)) {
			...
			throw ('source must be flash.display.BitmapData');
		}
		_vseg = vseg;
		_hseg = hseg;
	...
	function showId(evt:MouseEvent) {
		//trace(evt.localX + 'x' + evt.localY);
	...
	function renderVector (source:DisplayObject, offsetRect:Rectangle) :Void {
		...
		_container.transform.matrix.translate (	source.transform.matrix.tx,
												source.transform.matrix.ty);
		_w = offsetRect.width;
	...
	function __addTriangle (p0:SandyPoint, p1:SandyPoint, p2:SandyPoint) :Void {
		...
		tMat.c = _w / (v1 - v0);
		// --
		_tri.push ( new Triangle( p0, p1, p2, tMat, Std.string(_tri.length) ) );
	}
	...
	function __render() :Void {
		...
		if (grid != null) _container.removeChild ( grid );
		grid = new Sprite();
		c.addChild ( grid );
		//--
		var l = _tri.length;
...
*/	
	function __concat ( m1:Matrix, m2:Matrix ) :Matrix {
...
class SandyPoint extends Point {
